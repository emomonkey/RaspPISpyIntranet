...
def getActivity():
    ...
    #    scur.execute('SELECT datelogged, isdetection FROM aa_detections ')
        scur.execute('SELECT * FROM (SELECT datelogged, ? FROM aa_detections WHERE isdetection = 1 UNION SELECT datelogged, ? FROM aa_detections WHERE isdetection = 0 UNION SELECT datetime datelogged, comment FROM ac_imgcomments) ORDER BY datelogged DESC LIMIT 50', ['Sensor Alert', 'Random Photo'])
...
def sendemail(filenm, emailtext):
    msg = Message(
           ...
           sender='stephen.emo@gmail.com',
           recipients=
               ['stephen.emo@gmail.com'])
    msg.body = emailtext
...
def calculatesnaps() :
    ...
    global cfgSetup
    for x in range(1,cfgSetup.photo_freq):
        sval = random.randint(1,60)
...
def get_detections(pndate):
    spath = "static/images/SnapView/%s" % pndate  
...
def insert_db(bAlarm):
    with app.app_context():
        ...
        if row[0] == 0 or bAlarm:
            db.execute('insert into aa_detections (datelogged, isdetection) values (?, ?)', [vdate,boolcnv(bAlarm)])
            db.commit();
...
def autoshot():
    ...
    if cfgSetup.random_mode == 1 and ((sStartTime != "00:00" and sEndTime != "00:00") or (int(stime) > int(sStartTime) and int(stime) < int(sEndTime)  )):
        cfgSetup.icnt += 1
        if cfgSetup.icnt == cfgSetup.lsttime[0]:
            takecamera(False)
